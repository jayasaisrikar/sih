...
def generate_sentiment_pyramid(df):
    ...
    pyramid_filename = f'sentiment_pyramid_{time.time()}.png'
...
def barplot(df):
    ...
    barplot_filename = f'barplot_{time.time()}.png'  # Use a unique filename
...
def top(df):
    ...
    # Save the Plotly figure as an image (PNG or JPG)
    top_words_filename = f'top_words_{time.time()}.png'
